...
public class FirstRun implements InitializingBean {
    ...
    public void afterPropertiesSet() {
        ...
        Integer ret7 = RunSqlScript.RunSelectCount(judgeTable + " 'keys' and column_name = 'SysTransmit'");
        if (ret7 == 0) {
            RunSqlScript.RunInsert(sql7);
            Print.Normal("Add keys.SysTransmit");
        }
        Integer ret8 = RunSqlScript.RunSelectCount(judgeTable + " 'imgdata' and column_name = 'brieflink'");
        if (ret8 == 0) {
            RunSqlScript.RunInsert(sql8);
            Print.Normal("Add imgdata.brieflink");
        }
        Integer ret9 = RunSqlScript.RunSelectCount(judgeTable + " 'imgdata' and column_name = 'shortlink'");
        if (ret9 == 0) {
            RunSqlScript.RunInsert(sql9);
            Print.Normal("Add imgdata.shortlink");
        }
        Integer ret10 = RunSqlScript.RunSelectCount(" select count(*) from leona_timage.keys where storageType='9'");
        if (ret10 == 0) {
            RunSqlScript.RunInsert(sql10);
            Print.Normal("Add Webdav Config");
        }
        RunSqlScript.RunInsert(index_imgdata);
        RunSqlScript.RunInsert("ALTER TABLE leona_timage.`user` MODIFY id int auto_increment;");
    ...
    private String isTableName = "SELECT count(table_name) FROM information_schema.TABLES WHERE TABLE_SCHEMA='leona_timage' and table_name =";
    private String judgeTable = "select count(*) from information_schema.columns where TABLE_SCHEMA='leona_timage' and table_name = ";
    ...
    private String sql7 = "alter table `keys` add SysTransmit varchar(50) DEFAULT NULL ;";
    ...
    // 图片标识名字段
    private String sql10 = "INSERT INTO `leona_timage`.`keys`(`id`, `AccessKey`, `AccessSecret`, `Endpoint`, `Bucketname`, `RequestAddress`, `storageType`, `keyname`, `Region`, `RootPath`, `SysTransmit`) VALUES (null, '账号', '密码', '连接', '', '存储源请求域名', 9, 'WebDAV', NULL, '/', NULL);";

    private String sql12 = "alter table leona_timage.user add `token` varchar(255)";
    // 创建客户端程序相关表
    ...
    public boolean isWindows() {
        ...
        if (System.getProperties().getProperty("os.name").toUpperCase().contains("MAC")) {
            ...
            GlobalConstant.LOCPATH = props.getProperty("user.home") + File.separator + ".storage";
        }
